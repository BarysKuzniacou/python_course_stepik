...
# Используя моржовый оператор и переменную s, необходимо с помощью генератора списка сформировать новый список lst из 
# сумм текущего и всех предыдущих значений кортежа t. Например, для кортежа:
...
lst = [s := s + t[i] for i in range(len(t))]
