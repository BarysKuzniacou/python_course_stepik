...
# достичь риски под номером N (натуральное число N подается на вход программе).
...
def get_path(n, paths=0):
    if n == 1:
        return 1
    elif n == 2:
        return 2
    else:
        paths += get_path(n-1) + get_path(n-2) # get_path(n-1) + get_path(n-2)
