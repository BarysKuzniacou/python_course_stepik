...
# На вход программы поступает список целых чисел, записанных через пробел. Необходимо 
# прочитать эти числа и сохранить в списке digs. Затем, вызовите функцию filter_lst 
# несколько раз для формирования:
...
# определения анонимной функции. Каждый результат работы функции следует отображать 
# с новой строки командой:
...
lst = filter_lst(digs, lambda filter_key: filter_key < 0)
print(*lst)

lst = filter_lst(digs, lambda filter_key: filter_key >= 0)
print(*lst)

lst = filter_lst(digs, lambda filter_key: filter_key >= 3 and filter_key <= 5)
print(*lst)